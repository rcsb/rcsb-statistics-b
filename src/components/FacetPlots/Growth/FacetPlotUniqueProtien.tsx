...
export function FacetPlot(props: FacetPlotInterface) {
    ...
    const extractDataSets = (data: ChartObjectInterface[][]): Set<string> => {
        ...
        data.forEach((dataset, index) => {
            if (dataset.length > 0) { 
                const datasetId = datasetIds[index] || `Dataset-${index + 1}`;
                dataSetsShown.add(datasetId); 
                ...
                dataset.forEach(item => {
                    if (item.objectConfig) {
                        item.objectConfig.objectId.unshift(datasetId);
                        item.objectConfig.color = color;
                    } else {
                        item.objectConfig = {
                            objectId: [datasetId],
                            color: color
    ...
    const getSelectedDataText = (): string => {
        if (selectedDataSets.size === 2) {
            return "Cumulative (available each year) and annual number of released Unique Protien Sequences";
        } else if (selectedDataSets.has("Cumulative")) {
            return "Cumulative (available each year) number of released Unique Protien Sequences";
        } else if (selectedDataSets.has("Annual")) {
            return "Annual number of released Unique Protien Sequences";
        } else {
