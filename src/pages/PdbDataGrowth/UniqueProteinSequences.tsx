...
import { FacetPlot } from '../../components/FacetPlots/Growth/FacetPlotUniqueProtien';
...
interface DataGrowthNumberOfUniqueProteinSequencesState {
  mainAttribute: StatsFacetInterface;
  additionalAttribute?: StatsFacetInterface;
  selectedDataSet: string;
}

const UniqueProteinSequences: React.FC = () => {
  const [state, setState] = useState<DataGrowthNumberOfUniqueProteinSequencesState>({
  ...
  return state.mainAttribute.facet && state.mainAttribute.chartType ? (
    <Article>
      <h4>PDB Data Growth By Unique Protein Sequences</h4>
      <Container>
        <Row>
          <Col md={10}>
            <FacetPlot
...
export default UniqueProteinSequences;
